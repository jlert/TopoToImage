...
try:
    ...
    print("⚠️ reportlab not available - Key file generation disabled")
...
class KeyFileGenerator:
    ...
    def generate_key_file(self, export_data: Dict, output_path: str) -> bool:
        ...
        if not REPORTLAB_AVAILABLE:
            print("❌ Cannot generate Key file - reportlab not installed")
            return False
            
        try:
            print(f"📄 Creating PDF canvas: {output_path}")
            # Create PDF canvas
            c = canvas.Canvas(output_path, pagesize=letter)
            
            # Set up layers for Adobe Illustrator compatibility
            print("🎨 Setting up PDF layers for Adobe Illustrator compatibility")
            self._setup_pdf_layers(c)
            
            # Generate gradient bar image
            print("🌈 Generating gradient bar image")
            gradient_bar_image = self._create_gradient_bar_image(export_data)
            
            # Draw all elements
            print("📝 Drawing header section")
            self._draw_header(c, export_data)
            print("📊 Drawing unified info panel")
            self._draw_info_panel(c, export_data)
            print("📈 Drawing gradient legend section")
            self._draw_gradient_legend(c, export_data, gradient_bar_image)
            print("🎨 Drawing special colors section")
            self._draw_special_colors(c, export_data)
            
            # Save the PDF
            print("💾 Saving PDF file")
            c.save()
            print(f"✅ Key file generated: {output_path}")
            return True
            
        except Exception as e:
            print(f"❌ Error generating Key file: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _draw_header(self, c: "canvas.Canvas", export_data: Dict):
        ...
        # Line 2: Generated by with version and time (12pt normal)
        from version import get_app_name_with_version
        y -= 25  # Reduced spacing for 3 lines
        c.setFont("Helvetica", self.font_size_normal)

        generated_line = f"Generated by {get_app_name_with_version()}"
        if 'generation_time' in export_data:
            generated_line += f" in {export_data['generation_time']}"
        c.drawString(self.margin, y, generated_line)

        # Line 3: Saved on timestamp (12pt normal)
        y -= 18  # Standard line spacing
        if 'timestamp' in export_data:
            c.drawString(self.margin, y, f"Saved on: {export_data['timestamp']}")

        # Optional Line 4: File path (if space permits and path exists)
        if 'file_path' in export_data:
            y -= 18
            c.setFont("Helvetica", self.font_size_small)
            c.drawString(self.margin, y, f"File: {export_data['file_path']}")
    ...
    def _draw_elevation_labels(self, c: "canvas.Canvas", export_data: Dict, bar_x: float, bar_y: float):
        ...
        for i, color_stop in enumerate(gradient.color_stops):
            ...
            # This gives: First point Y = (360 × 0.05) + (536 - 360) = 18 + 176 = 194
    ...
    def _create_gradient_bar_image(self, export_data: Dict) -> Optional[Image.Image]:
        ...
        try:
            ...
            print(f"❌ Error creating gradient bar image: {e}")
            return None
